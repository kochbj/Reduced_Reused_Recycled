...
#mlm.gini_n2_full<-glmmTMB(gini_n2 ~ date*task_size*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters),data=cdata,family=beta_family(link = "logit"))
#mlm.gini_n1_full<-glmmTMB(gini_n ~ date*task_size*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters),data=cdata,family=beta_family(link = "logit"))
#mlm.gini_0_full<-glmmTMB(gini ~ full_size+date+task_size+task_age+CV*date+NLP*date+cvmethods*date,ziformula =~1, data=cdata,family=beta_family(link = "logit"))
mlm.gini_0_full<-glmmTMB(gini_n2 ~ date*task_size*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters), data=cdata,family=beta_family(link = "logit"))

mlm.gini_1<-glmmTMB(gini_n2 ~ date*task_size+date*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters), data=cdata,family=beta_family(link = "logit"))
mlm.gini_2<-glmmTMB(gini_n2 ~ date*task_size+date+task_age+CV*date+NLP*date+cvmethods*date+(1|clusters), data=cdata,family=beta_family(link = "logit"))
mlm.gini_3<-glmmTMB(gini_n2 ~ date+task_size+date*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters), data=cdata,family=beta_family(link = "logit"))
mlm.gini_4<-glmmTMB(gini_n2 ~ date+task_size+date+task_age+CV*date+NLP*date+cvmethods*date+(1|clusters), data=cdata,family=beta_family(link = "logit"))
compare_performance(mlm.gini_full, mlm.gini_1, mlm.gini_2,mlm.gini_3,mlm.gini_4)
test_performance(mlm.gini_full, mlm.gini_1, mlm.gini_2,mlm.gini_3,mlm.gini_4)

#mlm.gini_0_full<-glmmTMB(gini ~ date*task_size*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters),ziformula =~ date*task_size*task_age, data=cdata,family=beta_family(link = "logit"))
#gini_fits<-dredge(mlm.gini_0_full,extra = c(AIC,BIC,Cp),fixed=c("cond(date)"),trace=T)
#mlm.gini_0_n2<-glmmTMB(gini_n2 ~ date*task_size*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters), data=cdata,family=beta_family(link = "logit"))
#mlm.gini_0_n<-glmmTMB(gini_n ~ date*task_size*task_age+CV*date+NLP*date+cvmethods*date, data=cdata,family=beta_family(link = "logit"))

#plot(simulateResiduals(mlm.gini_0_n2,refit=F))
#plot(simulateResiduals(mlm.gini_0_n,refit=F))

#plot(simulateResiduals(mlm.gini_n1_full,refit=F))
plot(simulateResiduals(mlm.gini_full,refit=F))
plot(simulateResiduals(mlm.gini_n2_full,refit=F))
...
gini.custom<-emmip(mlm.gini_0_full,NLP+CV+cvmethods ~ date, at = mylist,CIs = T, type = 'response',plotit=F)
gini.custom<-gini.custom %>% filter(!(CV==1)) %>% filter(!(cvmethods==1))
gini.custom$NLP <- factor(gini.custom$NLP, levels=c(1,0))
levels(gini.custom$NLP)<-c("NLP","Marginal")
...
gini.custom<-emmip(mlm.gini_0_full,CV+NLP+cvmethods ~ date, at = mylist,CIs = T, type = 'response',plotit=F)
gini.custom<-gini.custom %>% filter(!(NLP==1)) %>% filter(!(cvmethods==1))
gini.custom$CV <- factor(gini.custom$CV, levels=c(1,0))
levels(gini.custom$CV)<-c("CV","Marginal")
...
gini.custom$Methods <- factor(gini.custom$cvmethods, levels=c(1,0))
levels(gini.custom$Methods)<-c("Methods","Marginal")
...
ggsave('./GoogleDataProject/EntropyInputs/GiniTrends.svg',gini.full)
ggsave('./GoogleDataProject/EntropyInputs/GiniTrends.png',gini.full)


#                        ,top = textGrob("Predicted Relationship between Time and Dataset Diversity within Tasks",gp=gpar(fontsize=16)))
emmip(mlm.gini_0_full,task_size ~ date, at = mylist,CIs = T, type = 'response',plotit=T)

fdata<-read_tsv('./GoogleDataProject/EntropyInputs/EntropyDatasetforRParentsOnly.AllYears.txt')
...
gini.aggregated
ggsave('./GoogleDataProject/EntropyInputs/GiniRatios.svg',gini.aggregated)
ggsave('./GoogleDataProject/EntropyInputs/GiniRatios.png',gini.aggregated)
ggplot(data=fdata,aes(x=size,y=adoption_pct))+
  geom_point()+
  geom_smooth()+
  scale_x_continuous(trans='log2')+
  scale_y_continuous(trans='log2') 
cor.test(fdata$task_size,fdata$adoption_pct,method = 'spearman')
