...
ratios.overtime.CV <-cdata %>% filter(task_size>10, CV==1) %>% group_by(year) %>% summarise(adopt.50=median(adoption_pct,na.rm=T),
                                                                                  adopt.25=quantile(adoption_pct,.25,na.rm=T), 
                                                                                  adopt.75=quantile(adoption_pct,.75,na.rm=T),
                                                                                  creation.50=mean(creation_pct,na.rm=T),
                                                                                  creation.25=quantile(creation_pct,.25,na.rm=T), 
...
ratios.overtime.NLP <-cdata %>% filter(task_size>10, NLP==1) %>% group_by(year) %>% summarise(adopt.50=median(adoption_pct,na.rm=T),
                                                                                            adopt.25=quantile(adoption_pct,.25,na.rm=T), 
                                                                                            adopt.75=quantile(adoption_pct,.75,na.rm=T),
                                                                                            creation.50=mean(creation_pct,na.rm=T),
                                                                                            creation.25=quantile(creation_pct,.25,na.rm=T), 
...
ratios.overtime.Methods <-cdata %>% filter(task_size>10, cvmethods==TRUE) %>% group_by(year) %>% summarise(adopt.50=median(adoption_pct,na.rm=T),
                                                                                               adopt.25=quantile(adoption_pct,.25,na.rm=T), 
                                                                                               adopt.75=quantile(adoption_pct,.75,na.rm=T),
                                                                                               creation.50=mean(creation_pct,na.rm=T),
                                                                                               creation.25=quantile(creation_pct,.25,na.rm=T), 
...
creation_ratio_full<-glmmTMB(creatRatio ~ date*task_size*task_age+CV*date+NLP*date+cvmethods*date,data=cdata,family='binomial')
...
creation.custom.NLP <- ggplot(data=creation.custom, aes(x=date,y=yvar, color=NLP)) +
...
creation.custom$Methods <- factor(creation.custom$cvmethods, levels=c(T,F))
...
adoption_ratio_full<-glmmTMB(adopRatio ~ date*task_size*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters),data=cdata,family='binomial')
xtable_output<-tidy(adoption_ratio_full) %>% select(-group,-component,-effect)
xtable_output$estimate<-exp(xtable_output$estimate)
xtable_output$std.error<-exp(xtable_output$std.error)
xtable_output$p.value<-round(xtable_output$p.value)
xtable_output$p.value<-round(xtable_output$p.value,4)
mylist <- list(task_size = quantile(cdata$task_size,p=c(.5)),date=1:(max(cdata$year)-min(cdata$year)+1),task_age=1:(max(cdata$year)-min(cdata$year)+1))

adoption.custom<-emmip(adoption_ratio_full,NLP+CV+cvmethods ~ date, at = mylist,CIs = T, type = 'response',plotit=F)
adoption.custom<-adoption.custom %>% filter(!(CV==1)) %>% filter(!(cvmethods==1))
...
adoption.custom<-emmip(adoption_ratio_full,CV+NLP+cvmethods ~ date, at = mylist,CIs = T, type = 'response',plotit=F)
adoption.custom<-adoption.custom %>% filter(!(NLP==1)) %>% filter(!(cvmethods==1))
...
adoption.custom$Methods <- factor(adoption.custom$cvmethods, levels=c(1,0))
...
adoption.custom.Methods <- ggplot(data=adoption.custom, aes(x=date,y=yvar, color=Methods)) +
  ...
  guides(color = "none")
xtable_output<-xtable(tidy(adoption_ratio_full)) %>% select(-group,-component,-effect) 

conRatio<-cbind(cdata$num_converted_growing,cdata$num_papers_growing)
conversion_ratio_full<-glmmTMB( conRatio ~ full_size+date*task_size*task_age+CV*date+NLP*date+cvmethods*date+(1|clusters),data=cdata,family='binomial')
#conversion_fits<-dredge(conversion_ratio_full,extra = c(AIC,BIC,Cp),fixed=c("cond(date)"))

conversion.custom<-emmip(conversion_ratio_full,NLP+CV+cvmethods ~ date, at = mylist,CIs = T, type = 'response',plotit=F)
conversion.custom<-conversion.custom %>% filter(!(CV==1)) %>% filter(!(cvmethods==T))
...
conversion.custom<-emmip(conversion_ratio_full,CV+NLP+cvmethods ~ date, at = mylist,CIs = T, type = 'response',plotit=F)
conversion.custom<-conversion.custom %>% filter(!(NLP==1)) %>% filter(!(cvmethods==T))
...
conversion.custom$Methods <- factor(conversion.custom$cvmethods, levels=c(T,F))
...
#                             ncol=3,nrow=3)
ratio.analyses<-grid.arrange(adoption.custom.CV,adoption.custom.NLP,adoption.custom.Methods)
